...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* 💻 메인 헤더 */}
      <header className="linear-font flex items-center justify-between px-6 h-14 bg-white dark:bg-[#0a0a0a] border-b border-gray-100 dark:border-gray-900 fixed inset-x-0 top-0 z-[10000] transition-colors duration-300">
        ...
        <div className="flex items-center h-full">
          ...
          <div className="hidden md:flex items-center gap-5 sm:gap-6">
            ...
            {/* 🚀 데스크탑 원상복구: 기존의 텍스트 형태로 얌전하게 되돌림 */}
            {user ? (
              <Link href="/profile" className="text-[11px] font-medium tracking-widest uppercase text-gray-500 dark:text-gray-400 transition-colors duration-200 hover:text-black dark:hover:text-gray-200">
                {t.profile}
              </Link>
            ) : (
              <Link href="/login" className="text-[11px] font-medium tracking-widest uppercase text-gray-500 dark:text-gray-400 transition-colors duration-200 hover:text-black dark:hover:text-gray-200">
                {t.login}
              </Link>
            )}
          ...
          {/* 📱 모바일 전용 영역 (데스크탑에서는 숨김) */}
          <div className="flex md:hidden items-center gap-3">
            
            {/* 🚀 모바일 전용: 알약(Pill) 스타일 유지 */}
            {user ? (
              <Link href="/profile" onClick={() => setIsOpen(false)} className="flex items-center justify-center px-3.5 py-1.5 rounded-full bg-[#f2f2f2] dark:bg-white/10 text-black dark:text-white text-[13px] font-medium hover:bg-[#e5e5e5] dark:hover:bg-white/20 transition-colors duration-200">
                Profile
              </Link>
            ) : (
              <Link href="/login" onClick={() => setIsOpen(false)} className="flex items-center justify-center px-3.5 py-1.5 rounded-full bg-[#f2f2f2] dark:bg-white/10 text-black dark:text-white text-[13px] font-medium hover:bg-[#e5e5e5] dark:hover:bg-white/20 transition-colors duration-200">
                Log in
              </Link>
            )}
            ...
            {/* 햄버거 버튼 */}
            <button 
              className="relative w-8 h-8 flex items-center justify-center text-black dark:text-white focus:outline-none -mr-1"
              onClick={() => setIsOpen(!isOpen)}
              aria-label="Toggle Menu"
            >
              <div className="relative w-4 h-4">
                <span 
                  className={`absolute left-0 top-1/2 w-full h-[1px] bg-current origin-center transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${
      ...
      {/* 🚀 모바일 풀스크린 오버레이 메뉴 */}
      <div 
        className={`linear-font md:hidden fixed inset-0 z-[9999] bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl pt-14 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] flex flex-col ${
      ...
      >
        <div className="flex flex-col flex-1 px-8 py-8 gap-10 overflow-y-auto pb-20">
          ...
          <div className={`flex flex-col gap-5 transition-all duration-700 delay-100 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
            ...
            <Link href={user ? "/profile" : "/login"} onClick={() => setIsOpen(false)} className="text-2xl font-medium tracking-tight text-black dark:text-white hover:opacity-70 transition-opacity">
