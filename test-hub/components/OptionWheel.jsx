...
// 서버사이드(Next.js) 에러 방지용 LayoutEffect
...
const OptionWheel = ({
  ...
  const lastTickIdxRef = useRef(defaultSelected);
  ...
  cfgRef.current = {
    count: items.length, items, rowH: Math.max(fontSize * spacing * remPx, 1),
    curve, tilt, blur, fade, minOpacity, side, loop, smoothing, draggable, soundUrl, soundVolume
  };

  // 🚀 React 간섭 없이 DOM에 직접 스타일을 주입하는 물리 엔진
  const layoutItems = useCallback((currentPos) => {
  ...
  // 🚀 화면 뭉침(FOUC) 완벽 차단: 브라우저가 화면을 그리기 직전에 즉시 배열을 끝냅니다!
  useIsoLayoutEffect(() => {
  ...
  const runFrame = useCallback(now => {
    ...
    layoutItems(next); // 계산된 위치를 즉시 반영

    const n = cfg.count;
    let currentRounded = Math.round(next);
    if (cfg.loop && n > 1) {
        currentRounded = ((currentRounded % n) + n) % n;
    } else {
        currentRounded = Math.max(0, Math.min(currentRounded, n - 1));
    }

    if (currentRounded !== lastTickIdxRef.current) {
        lastTickIdxRef.current = currentRounded;
  ...
  // 외부(page.tsx)에서 명령을 받으면 굴러갑니다.
  useEffect(() => {
    if (targetRef.current !== defaultSelected) {
  ...
  return (
    ...
    >
      {/* 🚀 React 간섭을 유발하던 inline style을 제거했습니다! 이제 안 고장납니다! */}
